...
I'm a fan of learning from examples so let's look at an example of what not keeping it simple looks like.
...
Step 1 is to applying the KISS principle, let's develop an outline for an algorithm that can be articulated in simple English:
...
> 3. Keep track of occurrences of `)` to "cancel out" a previous observation of `(`.
...
So we have developed our algorithm in simple English, which is essentially pseudocode! There are three logical components of our algorithm:
...
Let's move around the 6 steps of the pseudocode to these three logical components:
...
| Once we finish looking at the last character, if there's no leftover `(` or `)`, return true | Keep track of occurrences of `)` to "cancel out" a previous observation of `(` |                                                                            |
...
Separation of Concerns of SoC is the key to developing a complicated application or software system with many moving parts. Web applications separate concerns between presentation (how the webpage looks), business logic (content of the webpage), and content delivery (accessing the resource via a JSON API, querying the database, etc).
...
> Write a function foo that takes doubles then increments every number in an array of numbers
...
Why is this a poor execution of Separation of Concerns?
...
The Don't Repeat Yourself (DRY) principle and Separation of Concern go hand-in-hand. The DRY principle is aimed at reducing repetition and boilerplate in the software patterns by forming abstractions. I can write a whole separate article on abstractions but here are the key points for how to apply abstraction to write code that doesn't repeat itself:
...
* `filter` - get a subset of the array that passes a given criterion
...
When you hear "Divide and Conquer" in the context of algorithm design, think recursion. The term recursion has roots in mathematics. that uses a rule repeatedly to update itself ad infinitum. For example, fractals, the fibonacci sequence, and pascal's triangle are all recursive mathematical constructs.

For Divide and Conquer, we are interested in the repeated use of a rule but instead of constructing a sequence, we want to deconstruct the problem space and eventually return a final solution. In pseudo code, a recursive function for divide and conquer algorithm has the basic structure:

```
function recursiveFun(input, result) {
  if we hit a base case, updated result as necessary and return the final result

  otherwise, simplify the input. update the result
  return recursiveFun(simplerInput, updatedResult)
}
```

The emphasis is simplifying input until the problem becomes trivial to solve and we can return a result immediately in the beginning of the function. This is called the base case. What if we forget to add a base case to the recursive function? The function will keep calling itself forever until you run out of memory and get a stack overflow error.
...
function sum(arr) {
  ...
  // Initial Values
  let result = 0 // <-- this is a mutable data structure
  let elems = arr // <-- this is also a mutable data structure
...
In both Options 1 and 2, every time we repeat the procedure of adding the next element to the current sum, we are creating a simpler array and a updating the sum that's closer to the final solution. The stopping point is when we run out of things in the array to add to the current sum, in which case we just return the current sum and that's the final solution.
...
function isBalanced(str, openCnt) {
  ...
  // What to Look At Next?
	...
	return isBalanced(rst, newOpenCnt(fst, openCnt)) // <--  Divide and Conquer
}
...
**KISS** - Create a unit test function called `test` that performs unit testing on a function `fun` using a set of test cases. We want to be able to run a test for any function like so:
...
**DRY** - Abstract away the part of code that loops through all the testCases by using a higher order function `map`:
